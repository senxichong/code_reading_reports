...
```
在这里我想直接引用`Base class for all optimizers`这句话。这句话点出了'optimizer'这个类是抽象工厂类，其他的类型想要实现其他的功能只需要继承这个类，然后分别实现别的功能。
...
```
可以看到这里采用了抽象工厂模式，定义了抽象工厂之后，在具体工厂中实现了`Adam`优化器的具体功能。
因为面对多种优化器的需求，pytorch设计者没有选择一股脑地全部实现，因为需求是无穷无尽的，面对层出不穷的优化需求，各种各样的优化方法，不可能也没有必要全部实现。只实现一个基础的抽象工厂类，将最基本的基础功能实现，再在具体的工厂类中的实现这些具体的功能，比如惯性的引进，学习率的自适应等等功能。
