...
至此，我们分析了通过字节码创建一个 Test.class 文件涉及的主要类，这些类已经能够完成操作字节
码最基本的功能了，非常贴心的是，Javassist 项目单独编写了一个 ClassFileWriter 类，提供了创建一
个.class 类文件最小的 API，且 API 的命名都直观易懂，使用其中的 ConstPoolWriter，FieldWriter 和
MethodWriter 就能高效地构建出上面的 Test.class 文件。不得不说，项目在屏蔽底层细节、提升用户
体验上做的确实十分到位。